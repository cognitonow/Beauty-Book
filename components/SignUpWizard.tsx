...
const SignUpWizard: React.FC<SignUpWizardProps> = ({ onComplete, onNavigateToLogin, onViewLiveProfile, isOnboardingOnly = false, initialData = null }) => {
    ...
    useEffect(() => {
        if(isOnboardingOnly && initialData) {
            setAccountData(prev => ({ ...prev, id: initialData.id, name: initialData.name, email: 'loaded@user.com' }));
    ...
    const handleGenerateBio = async () => {
        if (!process.env.API_KEY) {
            alert("API_KEY is not configured. Cannot generate bio.");
            return;
        }
                        ...
                        <div className="mt-12 flex justify-between items-center w-full max-w-5xl mx-auto flex-shrink-0">
                           ...
                           {currentStep.step !== 6 ? (
                               <button 
                                    ...
                                    disabled={currentStep.step === 3 && !profileData.location}
                                    className="bg-rose-500 text-white font-bold py-3 px-8 rounded-lg hover:bg-rose-600 transition-colors disabled:bg-slate-300 disabled:cursor-not-allowed shadow-md hover:shadow-lg transform hover:scale-105">
