...
import { FileCode, X, Pin } from 'lucide-react';
...
const Header: React.FC<{
    ...
    onSave: () => void;
    onCancel: () => void;
}> = ({ name, isEditing, onSwitchRole, onEdit, onSave, onCancel }) => (
    <header className="py-4 px-6 bg-white/80 backdrop-blur-sm shadow-sm sticky top-0 z-20 flex justify-between items-center">
      ...
      <div className="flex items-center space-x-3">
        {isEditing ? (
            <>
                <button onClick={onCancel} className="text-sm font-semibold text-slate-600 hover:text-slate-900 px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">Cancel</button>
                <button onClick={onSave} className="text-sm font-semibold text-white px-4 py-2 bg-rose-500 hover:bg-rose-600 rounded-lg shadow-sm hover:shadow-md transition-all">Save Changes</button>
            </>
        ) : (
             <>
                ...
                <button onClick={onEdit} className="text-sm font-semibold text-white px-4 py-2 bg-rose-500 hover:bg-rose-600 rounded-lg shadow-sm hover:shadow-md">Edit Profile</button>
             </>
        )}
      </div>
    </header>
...
const ProfessionalView: React.FC<ProfessionalViewProps> = ({ profile, onUpdateProfile, onSwitchRole, onStartOnboarding }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [editData, setEditData] = useState<ProfessionalProfile | null>(null);

    useEffect(() => {
        // If the profile prop changes from the outside, and we are not in editing mode,
        // there is no need to update anything as the component will just re-render with the new prop.
        // If we ARE in editing mode, we don't want to overwrite the user's edits.
    }, [profile]);
    ...
    const handleEditClick = () => {
        setEditData(JSON.parse(JSON.stringify(profile))); // Deep copy for safe editing
        setIsEditing(true);
    };
    ...
    const handlePortfolioUpdate = (type: 'tiktok' | 'instagram', urls: string[]) => {
        ...
        // If the pinned URL was removed, unpin it.
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gradient-to-b from-rose-50 to-slate-50 font-sans">
            <Header
                name={profile.name}
                isEditing={isEditing}
            ...
            <main className="flex-grow p-6 space-y-6">
                {!profile.isProfileComplete && <IncompleteProfileBanner onStart={onStartOnboarding} />}
                
                {/* Stats Grid */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <StatCard label="Upcoming Bookings" value="5" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>} />
                    ...
                    <StatCard label="Total Reviews" value={totalReviews} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a2 2 0 01-2-2V7a2 2 0 012-2h4l4 4z" /></svg>} />
                </div>
